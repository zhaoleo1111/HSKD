...
# batch_size, data_dir, data
def imageset_loader(args):
    name = args.dataset
    ...
    if name == 'CUB200':
        ...
        raise NotImplementedError('Dataset {} is not prepared.'.format(args.data))
    ...
    transform_test = transforms.Compose([transforms.Resize(256), transforms.CenterCrop(224),
                                         transforms.ToTensor(), normalize, ])
    if args.method == 'DDGSD' or args.method == 'clsd':
        trainset = datasets.ImageFolder(train_val_dataset_dir, transform=TwoCropsTransform(transform_train))
    else:
        trainset = datasets.ImageFolder(train_val_dataset_dir, transform=transform_train)

    testset = datasets.ImageFolder(test_dataset_dir, transform=transform_test)
