...
class PyramidNet_aux(nn.Module):     
    def __init__(self,  depth, alpha, bottleneck=False):
        super(PyramidNet_aux, self).__init__()   	
        self.inplanes = 16
        if bottleneck == True:
            n = int((depth - 2) / 9)
            block = Bottleneck
...
class PyramidNet_aux2(nn.Module):     
    def __init__(self,  depth, alpha, bottleneck=False):
        super(PyramidNet_aux2, self).__init__()   	
        self.inplanes = 16
        if bottleneck == True:
            n = int((depth - 2) / 9)
            block = Bottleneck
...
class PyramidNet(nn.Module):     
    def __init__(self,  depth, alpha, bottleneck=False,num_classes=100, dataset='cifar100',branch_layers=[]):
        ...
        if self.dataset.startswith('cifar'):
            ...
            if len(self.branch_layers) != 0:
                self.aux = PyramidNet_aux(depth,alpha)
                self.aux1_layer2 = self.aux.layer2
                self.aux1_layer3 = self.aux.layer3
                self.bn_final1 = nn.BatchNorm2d(80)
                self.relu_final1 = nn.ReLU(inplace=True)
                self.fc1 = nn.Linear(80, num_classes)
                
                self.aux2 = PyramidNet_aux2(depth,alpha)
                self.aux2_layer3 = self.aux2.layer3
                self.bn_final2= nn.BatchNorm2d(80)
                self.relu_final2 = nn.ReLU(inplace=True)
    ...
    def forward(self, x, feature=False, feature_emd=False, embedding=False):
        if self.dataset == 'cifar10' or self.dataset == 'cifar100':
            ...
            if len(self.branch_layers) != 0:
                x_out = self.aux1_layer2(out1)
                x_out = self.aux1_layer3(x_out)
                x_out = self.bn_final1(x_out)
                x_out = self.relu_final1(x_out)
                f2 = x_out
                x_out = self.avgpool(x_out)
                x_out = x_out.view(x_out.size(0), -1)
                x2 = self.fc1(x_out)

                x_out = self.aux2_layer3(out2)
                x_out = self.bn_final2(x_out)
                x_out = self.relu_final2(x_out)
                f1 = x_out
                x_out = self.avgpool(x_out)
                x_out = x_out.view(x_out.size(0), -1)
                x1 = self.fc2(x_out)
...
def PyramidNet110(pretrained=False, **kwargs):
    return PyramidNet(depth=110, alpha=48, **kwargs)

def PyramidNet110_byot(pretrained=False, **kwargs):
    return PyramidNet(depth=110, alpha=48, branch_layers=[[1,1,1], [1]], **kwargs)