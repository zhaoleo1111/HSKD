...
def MixupFR(net, inputs, targets, criterion_cls, alpha):
    mixed_x, y_a, y_b, lam_mixup, index = mixup_data(inputs, targets, alpha=alpha)
    logit,feat = net(mixed_x,feature=True)
    if isinstance(logit, list) or isinstance(logit, tuple):
        logit = logit[0] 
    loss = criterion_cls(logit, y_a) * lam_mixup + criterion_cls(logit, y_b) * (1. - lam_mixup)
    return logit,feat,loss,lam_mixup,index 
