...
def train(epoch, criterion_list, optimizer,pre_data, pre_out):
    ...
    if args.method.startswith('PSKD') or args.method.startswith('psmgf') :
        if epoch == 0:
        ...
        if epoch == 0:
            ...
            all_predictions = torch.load(os.path.join(args.checkpoint_dir, 'predictions.pth.tar'), map_location=torch.device('cpu'))['prev_pred']
            b1_predictions = torch.load(os.path.join(args.checkpoint_dir, 'b1_predictions.pth.tar'), map_location=torch.device('cpu'))['prev_pred']
            b2_predictions = torch.load(os.path.join(args.checkpoint_dir, 'b2_predictions.pth.tar'), map_location=torch.device('cpu'))['prev_pred']
            b3_predictions = torch.load(os.path.join(args.checkpoint_dir, 'b3_predictions.pth.tar'), map_location=torch.device('cpu'))['prev_pred']
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if args.method == 'cross_entropy':
        ...
        elif args.method.startswith('uskd'):
            logit,feature = net(inputs,feature=True)
            mid_fea = feature[2]
            loss_kd = uskd(feature[2],logit,targets)
            loss_cls += criterion_cls(logit, targets)
            loss_div += loss_kd
            ...
            _,mixcl_loss = Mixup(net, inputs, targets, criterion_cls, alpha=0.4)
            loss_cls += mixcl_loss

        elif args.method ==  'dlb': 
            logit = net.forward(inputs[:, 0, ...])
          
            if pre_data != None:
                pre_images, pre_label = pre_data[0],pre_data[1]
                pre_images = pre_images.cuda()
                pre_label = pre_label.cuda()
                
                out_pre = net.forward(pre_images[:, 1, ...])
                ce_loss = F.cross_entropy(
                    torch.cat((out_pre, logit), dim=0), torch.cat((pre_label, targets), dim=0)
                )  
                
                dml_loss = (
                    F.kl_div(
                        F.log_softmax(out_pre / args.dl_T, dim=1),
                        F.softmax(pre_out.detach() / args.dl_T, dim=1),  # detach
                        reduction="batchmean",
                    )
                    * args.dl_T
                    * args.dl_T
                ...
                loss_cls += F.cross_entropy(logit, targets)
               
        
            pre_data = [inputs.detach(),targets.detach()]
            pre_out = logit
    ...
    if args.method.startswith('PSKD') or args.method.startswith('psmgf'):
        ...
        torch.save({'prev_pred': all_predictions.cpu()}, os.path.join(args.checkpoint_dir, 'predictions.pth.tar'))
        torch.save({'prev_pred': b1_predictions.cpu()}, os.path.join(args.checkpoint_dir, 'b1_predictions.pth.tar'))
        torch.save({'prev_pred': b2_predictions.cpu()}, os.path.join(args.checkpoint_dir, 'b2_predictions.pth.tar'))
        torch.save({'prev_pred': b3_predictions.cpu()}, os.path.join(args.checkpoint_dir, 'b3_predictions.pth.tar'))
    
    return pre_data, pre_out
...
if __name__ == '__main__':
    ...
    if args.evaluate:
        ...
        if args.method ==  'mgf' or args.method ==  'mixmgf' or args.method ==  'psmgf':
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            t0_time = time.time()
            pre_data, pre_out = train(epoch, criterion_list, optimizer,pre_data, pre_out)
            t1_time = time.time()
