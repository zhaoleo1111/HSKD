...
import time
import math
from wrapper_mgd import wrapper
from wrapper_cl import clwrapper
from losses import SupConLoss
from methods.ps_kd import HSD

parser = argparse.ArgumentParser(description='PyTorch CIFAR Training')
parser.add_argument('--dataset', default=' CUB200', type=str, help='Dataset')
...
parser.add_argument('--dl_T', type=float, default=4.0, help='temperature for KD distillation')
...
if args.method == 'dlb' or args.method == 'PSKD' or args.method == 'hskd':
...
# Training
def train(epoch, criterion_list, optimizer,pre_data, pre_out):
    ...
    if args.method.startswith('hskd'):
        if epoch == 0:
            is_pre = False
        else:
            is_pre = True
            checkpoint = torch.load(os.path.join(args.checkpoint_dir, 'lastmodel.pth.tar'), map_location=torch.device('cpu'))
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if args.method == 'cross_entropy':
        ...
        elif args.method.startswith('hskd'):
            inputs1 = inputs[:, 0, ...]
            inputs2 = inputs[:, 1, ...]
            mix_inputs, targets_a, targets_b, lam, index = mixup_data(inputs1, targets, alpha=0.4)
            
            logit,ori_feat = net(mix_inputs,feature=True)
            loss_cls += mixup_criterion(criterion_cls, logit, targets_a, targets_b, lam)
            #####mask_generate######
            f_out = rec_encoder(ori_feat[1])
            mixup_proj = cl_projector(ori_feat[-1])
            #####mask_generate######
            aux_loss = (
                    F.kl_div(
                        F.log_softmax(f_out/ args.kl_T, dim=1),
                        F.softmax(logit/ args.kl_T, dim=1),
                        reduction="batchmean",
                    )
                    * args.kl_T
                    * args.kl_T)
            ...
            if is_pre:
                with torch.no_grad():
                    #####only_use_aug####
                    # _,pre_feat = last_model(inputs1,feature=True)
                    # pre_out_aug = last_model(inputs2) 
                    # last_out = pre_out_aug.cpu().detach()
                    ########ori###
                    pre_out,pre_feat = last_model(inputs1,feature=True)
                    pre_out_aug = last_model(inputs2) 
                    last_out = pre_out.cpu().detach()
                    ########ori###
                ##先mix_feat再proj###
                mix_feat = lam * (pre_feat[-1]) + (1 - lam) * (pre_feat[-1][index])
                mix_pre_proj = cl_projector(mix_feat)
                features = torch.cat([mix_pre_proj.unsqueeze(1),mixup_proj.unsqueeze(1)],dim=1)
                cl_loss =  criterion_cl(features)  
                ######对辅助分支使用历史信息进行监督############
                mix_kl = HSD(f_out,targets_a, targets_b, epoch, last_out, num_classes,index,lam,args)
                hist_loss1 = (
                    F.kl_div(
                        F.log_softmax(logit/ args.dl_T, dim=1),
                        F.softmax(pre_out_aug.detach()/ args.dl_T, dim=1),
                        reduction="batchmean",
                    )
                    * args.dl_T
                    * args.dl_T)
                ...
                hist_loss2 = (
                    F.kl_div(
                        F.log_softmax(logit/ args.dl_T, dim=1),
                        F.softmax(pre_out_aug[index].detach()/ args.dl_T, dim=1),
                        reduction="batchmean",
                    )
                    * args.dl_T
                    * args.dl_T)
                hist_loss = lam *hist_loss1 + (1 - lam) * hist_loss2
                #over_all total_loss####
                loss_div += 0.5*(aux_loss + mix_kl) + 0.2*cl_loss + 0.5*hist_loss
            else:
                loss_div += 0.5*aux_loss
    ...
    if args.method.startswith('hskd'):
        torch.save(net.state_dict(), os.path.join(args.checkpoint_dir, 'lastmodel.pth.tar'))
...
if __name__ == '__main__':
    ...
    criterion_cl = SupConLoss(temperature=0.3)
    criterion_list = nn.ModuleList([])
    criterion_list.append(criterion_cls)  # classification loss
    criterion_list.append(criterion_div)
    criterion_list.append(criterion_mse)
    criterion_list.append(criterion_cl)
    criterion_list.cuda()
    ...
    if args.evaluate:
        ...
        if args.method ==  'hskd':
            ...
            feat_H = feat[-2].shape[2]
            rec_encoder = wrapper(feat_H,num_channels1,num_channels2,num_channels3,num_channels4,num_in,num_classes)
            cl_projector = clwrapper(num_in)
            trainable_list.append(rec_encoder)
            trainable_list.append(cl_projector)
            trainable_list.cuda()
        else:
            trainable_list = nn.ModuleList([])
            trainable_list.append(net)
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            t0_time = time.time()
            train(epoch, criterion_list, optimizer,pre_data, pre_out)
            t1_time = time.time()
